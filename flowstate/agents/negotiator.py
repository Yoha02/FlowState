...
async def run(state: SharedSentinelState):
    """
    Wait for handoff_trigger -> fire modal -> wait for approval/timeout.

    Returns True if approved, raises RuntimeError if timed out/rejected.
    Called by _run_handoff_pipeline in orchestrator.
    """
    ...
    # Wait for user response (generous timeout for demo)
    try:
        ...
        raise RuntimeError("Handoff timed out")
